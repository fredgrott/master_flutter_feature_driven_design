...
import 'package:fdd_api/core/domain_events/domain_event.dart';
import 'package:fdd_api/core/domain_events/domain_event_listener.dart';
...
class DomainEventDispatcher {
  ...
  Future<void> dispatch(DomainEvent event) async {
    final List<DomainEventListener<DomainEvent>>? eventListeners = _listeners[event.runtimeType];
    if (eventListeners != null) {
      for (final listener in eventListeners) {
        try {
          ...
          debugPrint('Error in listener $listener for event $event: $e');
        }
      }
    }
  }
  ...
  void unregisterListener<T extends DomainEvent>(DomainEventListener<T> listener) {
    final Type eventType = T;
    final List<DomainEventListener<DomainEvent>>? eventListeners = _listeners[eventType];
